<x-default-layout>
    @section('title', 'Konfigurasi Siklus PI')

    @section('breadcrumbs')
        {{ Breadcrumbs::render('siklus2.configure', $siklus) }}
    @endsection

    <div class="card shadow-sm">
        <div class="card-header">
            <div class="card-title">Konfigurasi Siklus PI: {{ $siklus->nama }}</div>
        </div>
        <div class="card-body">
            @if(session('success'))
            ...
            <div class="alert alert-info">
                <div class="d-flex">
                    ...
                    <div>
                        ...
                        <p>Pilih mata kuliah untuk setiap PI yang akan digunakan dalam perhitungan ketercapaian PI pada siklus ini. Hanya mata kuliah yang memiliki keterhubungan dengan PI yang akan ditampilkan.</p>
                        <p>Perhitungan rata-rata PI akan menggunakan nilai CPMK-PI dari mata kuliah yang dipilih dalam rentang tahun siklus ({{ $siklus->tahun_mulai }} - {{ $siklus->tahun_selesai }}).</p>
                    </div>
                </div>
            </div>

            <form action="{{ route('siklus2.save-pi-selections', $siklus) }}" method="POST" id="configure-form">
                @csrf

                @foreach($cpls as $cpl)
                    <div class="card mb-6">
                        <div class="card-header">
                            <h3 class="card-title">PI {{ $cpl->cpl->nomor .'-'. $cpl->nomor }} </h3>
                        </div>
                        <div class="card-body">
                            <div class="mb-3">
                                <p class="text-gray-600">{!! nl2br(e('CPL '. $cpl->cpl->nomor .' : '. $cpl->cpl->nama )) !!}</p>
                            </div>

                            <div class="mb-5">
                                <label class="form-label">Pilih Mata Kuliah untuk PI ini:</label>
                                            ...
                                            @else
                                                <tr>
                                                    <td colspan="5" class="text-center text-muted">
                                                        Tidak ada mata kuliah yang terhubung dengan PI ini
                                                    </td>
                                                </tr>
                                            @endif
                                        </tbody>
                ...
                <div class="d-flex justify-content-end">
                    <a href="{{ route('siklus2.index') }}" class="btn btn-light me-3">Kembali</a>
